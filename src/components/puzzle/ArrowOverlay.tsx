...
// which way the board is facing. Ported from Repertoire Lab's BoardArrows.
...
/**
 * Absolutely-positioned SVG overlay drawn on top of a board, sized to match
 * it via ResizeObserver. Knight moves bend at a right angle the way
 * chess.com draws them, instead of cutting across diagonally.
 */
export function ArrowOverlay({
  ...
  return (
    <div ref={containerRef} className="pointer-events-none absolute inset-0" style={{ zIndex: 10 }}>
      {arrows.length > 0 && boardWidth > 0 && (
        <ArrowSvg arrows={arrows} boardWidth={boardWidth} orientation={orientation} />
      )}
    </div>
  )
}
...
function ArrowSvg({
  ...
  const centre = (c: { x: number; y: number }) => ({ x: (c.x + 0.5) * size, y: (c.y + 0.5) * size })
  ...
  const tailInset = size * 0.36

  return (
    <svg width={boardWidth} height={boardWidth} viewBox={`0 0 ${boardWidth} ${boardWidth}`}>
      {arrows.map((arrow, i) => {
        const a = cell(arrow.startSquare, orientation)
        const b = cell(arrow.endSquare, orientation)
        const dx = b.x - a.x
        const dy = b.y - a.y
        ...
        const end = centre(b)
        const stroke = arrow.color || '#e8b339'
        ...
        if (isKnightHop(dx, dy)) {
          corner = Math.abs(dy) > Math.abs(dx) ? centre({ x: a.x, y: b.y }) : centre({ x: b.x, y: a.y })
        }
        ...
        // The tip sits on the target's centre; the shaft stops where the head begins.
        ...
        return (
          <g key={`${arrow.startSquare}-${arrow.endSquare}-${i}`} opacity={0.85}>
            <polyline points={points} fill="none" stroke={stroke} strokeWidth={shaft} strokeLinecap="butt" strokeLinejoin="miter" />
            <polygon
